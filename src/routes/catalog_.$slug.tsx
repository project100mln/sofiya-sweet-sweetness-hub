...
export const Route = createFileRoute("/catalog_/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) return { meta: [{ title: "Товар не найден | SOFIYA" }, { name: "robots", content: "noindex" }] };
    ...
    <>
      <div className="container-page pt-8">
        <nav className="text-sm text-muted-foreground flex items-center gap-1 flex-wrap">
          ...
          {category.slug && (<>
            <ChevronRight className="h-3.5 w-3.5" />
            <Link to="/catalog" search={{ cat: category.slug }} className="hover:text-primary">{category.name}</Link>
          </>)}
          <ChevronRight className="h-3.5 w-3.5" />
      ...
      <section className="container-page py-8 md:py-12 grid gap-10 lg:grid-cols-2">
        ...
        <div>
          <div className="flex flex-wrap gap-2">
            {p.isHero && <span className="rounded-full bg-primary text-primary-foreground px-3 py-1 text-xs font-semibold uppercase tracking-wider">Хит</span>}
            {p.isNew && <span className="rounded-full bg-[color:var(--gold)] text-foreground px-3 py-1 text-xs font-semibold uppercase tracking-wider">Новинка</span>}
            {p.isPreorder && <span className="rounded-full bg-accent text-accent-foreground px-3 py-1 text-xs font-semibold uppercase tracking-wider">На заказ</span>}
          </div>
          ...
          {p.price != null && (
            <p className="mt-6 text-3xl font-bold text-primary">{p.price.toLocaleString("ru-RU")} ₸</p>
          )}
          ...
          {p.ingredients && p.ingredients.length > 0 && (
            <div className="mt-6">
              <h3 className="text-sm font-semibold uppercase tracking-widest text-muted-foreground">Состав</h3>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {p.isPreorder && (
              <Link to="/cake-preorder" className="btn-primary btn-primary-hover">Оформить предзаказ</Link>
            )}
            {site.whatsappDigits && (
              <a href={waLink(`Здравствуйте, SOFIYA! Меня интересует «${p.name}».`)} target="_blank" rel="noreferrer" className="btn-outline btn-outline-hover">
                <MessageCircle className="h-4 w-4" /> Спросить в WhatsApp
              </a>
            )}
            <a href={site.instagramUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full px-4 py-3 text-sm font-semibold text-primary hover:text-primary-hover">
