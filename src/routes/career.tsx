...
export const Route = createFileRoute("/career")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Работайте в растущей сети SOFIYA: пекари, кондитеры, бариста, менеджеры. Обучение и развитие." },
    ],
  }),
    ...
    <>
      <section className="relative overflow-hidden bg-gradient-to-br from-primary to-[color:var(--secondary)] text-primary-foreground">
        <div className="container-page py-14 md:py-20">
          ...
          <p className="mt-4 text-lg text-white/85 max-w-2xl">Мы — растущая сеть кофеен и пекарен. Ищем людей, которые любят своё дело и хотят расти вместе с нами.</p>
        </div>
      </section>
      ...
      <section className="container-page py-6">
        <div className="rounded-3xl bg-[color:var(--accent)] p-8 md:p-12">
          ...
          <p className="mt-3 text-muted-foreground max-w-xl">Список вакансий скоро появится. Пока оставьте заявку — мы свяжемся с вами, как только откроем позицию в вашем городе.</p>
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {[
              ...
              <div key={t} className="rounded-2xl bg-background border border-border p-4 flex items-center gap-3">
        ...
        <div className="mx-auto max-w-2xl rounded-3xl bg-card border border-border p-6 md:p-10">
          ...
          {sent ? (
            ...
            <form onSubmit={submit} className="mt-6 grid gap-4">
              <label className="block"><span className="text-sm font-semibold block mb-1.5">Направление *</span>
                <select required value={f.position} onChange={(e) => set("position", e.target.value)} className="input">
                  <option value="">Выберите</option>
                  {["Кондитер / Пекарь", "Бариста", "Менеджер точки", "Другое"].map(x => <option key={x}>{x}</option>)}
                </select>
              </label>
              ...
              <label className="block"><span className="text-sm font-semibold block mb-1.5">Город *</span><input required value={f.city} onChange={(e) => set("city", e.target.value)} className="input" placeholder="Шымкент" /></label>
              <label className="block"><span className="text-sm font-semibold block mb-1.5">О себе</span><textarea rows={4} value={f.comment} onChange={(e) => set("comment", e.target.value)} className="input min-h-[100px]" /></label>
