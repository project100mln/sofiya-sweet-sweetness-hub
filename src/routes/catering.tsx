...
export const Route = createFileRoute("/catering")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Кейтеринг SOFIYA: кофе-брейки, десертные столы, корпоративные события и большие заказы." },
    ],
  }),
      ...
      <section className="container-page py-12">
        <div className="grid gap-4 md:grid-cols-3">
          {SERVICES.map(({ i: Icon, t, d }) => (
            <div key={t} className="rounded-3xl bg-card border border-border/60 p-6 hover:border-primary/40 hover:shadow-soft transition-all">
        ...
        <div className="mx-auto max-w-2xl rounded-3xl bg-card border border-border p-6 md:p-10">
          ...
          {sent ? (
            <div className="mt-8 text-center">
              ...
              <p className="mt-2 text-muted-foreground text-sm">Мы открыли ваш WhatsApp с готовым сообщением.</p>
            </div>
          ) : (
            <form onSubmit={submit} className="mt-6 grid gap-4">
              <Row label="Тип события" required>
                <select value={f.type} onChange={(e) => set("type", e.target.value)} required className="input">
                  <option value="">Выберите</option>
                  {SERVICES.map(s => <option key={s.t} value={s.t}>{s.t}</option>)}
                </select>
              </Row>
              <div className="grid sm:grid-cols-2 gap-4">
                <Row label="Дата" required><input type="date" min={new Date().toISOString().split("T")[0]} required value={f.date} onChange={(e) => set("date", e.target.value)} className="input" /></Row>
                <Row label="Гостей" required><input type="number" min={1} required value={f.guests} onChange={(e) => set("guests", e.target.value)} className="input" placeholder="20" /></Row>
              </div>
              <Row label="Бюджет (примерно)"><input value={f.budget} onChange={(e) => set("budget", e.target.value)} className="input" placeholder="Например: 50 000 ₸" /></Row>
              <div className="grid sm:grid-cols-2 gap-4">
                <Row label="Имя" required><input required value={f.name} onChange={(e) => set("name", e.target.value)} className="input" /></Row>
                <Row label="Телефон" required><input type="tel" required value={f.phone} onChange={(e) => set("phone", e.target.value)} className="input" placeholder="+7 ___ ___ __ __" /></Row>
              </div>
              <Row label="Комментарий"><textarea rows={4} value={f.comment} onChange={(e) => set("comment", e.target.value)} className="input min-h-[100px]" /></Row>
              <button type="submit" className="btn-primary btn-primary-hover mt-2">Отправить заявку</button>
            </form>
          )}
        </div>
      </section>
...
function Row({ label, required, children }: { label: string; required?: boolean; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="text-sm font-semibold text-foreground mb-1.5 block">{label} {required && <span className="text-primary">*</span>}</span>
      {children}
    </label>
  );
}
